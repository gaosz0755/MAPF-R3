...
class Environment:
    def __init__(self, escape_policy, num_agents: int = config.init_env_settings[0], map_length: int = config.init_env_settings[1],
        ...
        self.os_random_pick = escape_policy
    ...
    def step(self, actions: List[int]):
        ...
        for i in range(self.num_agents):
            if np.array_equal(self.agents_pos[i], self.goals_pos[i]):
                ...
                rewards[i] = self.reward_fn['finish']
    ...
    def observe(self):
        ...
        for i, agent_pos in enumerate(self.agents_pos):
            x, y = agent_pos
            temp_map = self.map
            temp_map[self.goals_pos[i][0], self.goals_pos[i][1]] = 0
            obstacle_map = np.pad(temp_map, self.obs_radius, 'constant', constant_values=0)
