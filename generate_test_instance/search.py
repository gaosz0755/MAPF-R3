...
# use A_star to find path 
def find_path(map, starts, goals):
    get_solution = True
    root = {'cost': 0,
            'constraints': [],
            'paths': [],
    ...
    heuristics = []
    for goal in goals:
        heuristics.append(compute_heuristics(map, goal))

    for i in range(len(starts)):  # Find initial path for each agent
        temp_map = copy.deepcopy(map)
        # temp_map[goals[i]] = 0
        if starts[i] == goals[i]:
            print('starts[i] == goals[i]')
            print(starts[i])
            print(goals[i])
        path = a_star(temp_map, starts[i], goals[i], heuristics[i], i, root['constraints'], root['paths'])
